<template>
  <div class="page-sidebar-content-wrapper page-sidebar-left  small-mt__40 tablet-mt__40">
      ...
      <form method="post">
        <div class="widget-search">
          ...
          <button type="submit" class="search-submit">
            <span class="search-btn-icon fa fa-search" />
          </button>
        </div>
      </form>
    </div>
    ...
    <div class="sidebar-widget widget-tag wow move-up">
      <h4 class="sidebar-widget-title">
        {{ sidebarWidget.tagTitle }}
      </h4>
      <n-link v-for="tag in tags" :key="tag.id" :to="tag.url" class="ht-btn ht-btn-xs">
        {{ tag.name }}
      </n-link>
    </div>
    <!-- === Sidebar Widget End === -->
